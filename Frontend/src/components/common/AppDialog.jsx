...
function AppDialog({
  ...
  return (
    <div className="app-dialog-overlay" role="presentation">
      <div className={`app-dialog-card tone-${tone}`} role="dialog" aria-modal="true" aria-label={title || meta.badge}>
        ...
        <div className="app-dialog-actions">
          {showCancel ? (
            <button type="button" className="app-dialog-btn secondary" onClick={onCancel}>
              {cancelLabel}
            </button>
          ) : null}
          <button type="button" className={`app-dialog-btn primary tone-${tone}`} onClick={onConfirm}>
            {confirmLabel}
          </button>
        </div>
      </div>
