...
import { CheckCircle, AlertCircle, ShieldCheck, ArrowRight } from 'lucide-react';
...
export default function VerifyEmail() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const endpoint = role.toLowerCase() === 'hr' ? '/hrs/verify-email' : '/candidates/verify-otp';
                ...
                if (role.toLowerCase() === 'hr') {
                    navigate('/hrs/login', {
                        state: {
                            ...
                            successMessage: response.data.message || 'Email verified successfully. Your account is now waiting for admin approval.'
                        }
                    });
                } else {
    ...
    const handleResendOtp = async () => {
        if (!email || role.toLowerCase() === 'hr') {
            setStatus({ type: 'error', msg: "Resend OTP is only available for candidate verification right now." });
            return;
        }
        ...
        try {
            ...
            setStatus({
                type: 'error',
                msg: err.response?.data?.error || 'Failed to resend OTP. Please try again.'
            });
        } finally {
            setResending(false);
        }
        ...
        <div className="vfy-page">
            <div className="vfy-card">
                ...
                <p className="vfy-subtitle">
                    We've sent a 6-digit OTP to <span className="vfy-email-highlight">{email}</span>.
