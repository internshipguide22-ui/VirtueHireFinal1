...
const LiveAssessments = ({ hr, refreshTrigger, apiBase = '/hrs' }) => {
  ...
  const fetchLiveAssessments = async () => {
    ...
    try {
      ...
      const res = await api.get(`${apiBase}/assessments/live`, { withCredentials: true });
  ...
  const toggleLock = async (ast) => {
    ...
    try {
      setError(null);
      await api.put(`${apiBase}/assessments/${ast.id}/lock?lock=${newLockStatus}`, {}, { withCredentials: true });
      setAssessments(prev => prev.map(a => 
        a.id === ast.id ? { ...a, isLocked: newLockStatus } : a
      ));
    } catch (err) {
      const message = err.response?.data?.error || "Failed to update test status.";
      setError(message);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      setError(null);
      await api.delete(`${apiBase}/assessments/${testToDelete.id}`, { withCredentials: true });
