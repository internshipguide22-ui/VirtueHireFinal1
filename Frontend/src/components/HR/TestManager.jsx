...
const SectionRow = ({ section, index, allSubjectsInfo, onChange, onRemove }) => {
  ...
  const maxAvailable = selectedSubjectInfo
    ...
    : 0;
  const availableLabel = section.sectionMode === 'COMPILER' ? 'coding/compiler' : 'no-compiler';
  ...
  return (
    <div className="tm-section-row">
        ...
        >
          ...
          {allSubjectsInfo.map(s => (
            <option key={s.subject} value={s.subject}>
              {s.subject} ({s.noCompilerCount ?? s.count} no compiler, {s.compilerCount ?? 0} compiler)
            </option>
          ))}
        </select>
      </div>
      ...
      <div className="tm-section-ctrl">
        <label className="tm-ctrl-label">Questions (max {maxAvailable} {availableLabel})</label>
        <input
        ...
        <div className="tm-section-ctrl tm-section-ctrl-wide">
          ...
          <div className="tm-language-grid">
            {COMPILER_LANGUAGE_OPTIONS.map(language => {
              ...
              return (
                <label key={language} className={`tm-language-chip ${checked ? 'active' : ''}`}>
                  <input
                    type="checkbox"
                    checked={checked}
                    onChange={() => {
...
const TestManager = ({ hr, onSuccess, apiBase = '/hrs' }) => {
  ...
  const fetchSubjectsInfo = useCallback(async () => {
    try {
      const res = await api.get(`${apiBase}/subjects-info`, { withCredentials: true });
  ...
  const getUploadStatusMeta = (status) => {
    ...
    const authIssue = /unauthorized|bearer token|token missing|token invalid|not logged in/i.test(message);
  ...
  const validateAndSetFile = (f) => {
    ...
    const isCsv = fileName.endsWith('.csv') || fileType === 'text/csv' || fileType.includes('csv');
    const isPdf = fileName.endsWith('.pdf') || fileType === 'application/pdf' || fileType.includes('pdf');

    if (!isCsv && !isPdf) {
      addToast('Please upload a PDF or CSV file.', 'error'); return;
    }
    setFile(f);
    ...
    return new Promise((resolve, reject) => {
      Papa.parse(content, {
        ...
        complete: (results) => {
          ...
          const hasAnyHeader = (...keys) => keys.some((key) => normalizedHeaders[key]);
          ...
          const hasModernFormat =
            subjectFormatMissing.length === 0;
          const hasLegacyFormat =
            questionFormatMissing.length === 0;
          ...
          const activeFormat = hasMixedFormat ? 'mixed' : (hasModernFormat ? 'subject' : 'question');

          if (activeFormat === 'question' && !uploadTestName.trim()) {
            reject('Enter a Target Subject Name before uploading the question format CSV.');
            return;
          }
          ...
          for (let index = 0; index < rows.length; index += 1) {
            ...
            const subject = activeFormat === 'subject'
              ? getValue('subject')
              : activeFormat === 'mixed'
                ? (getValue('subject') || uploadTestName.trim())
                : uploadTestName.trim();
            const text = activeFormat === 'subject' ? getValue('text') : getValue('question');
            const options = activeFormat === 'subject'
              ? [getValue('option1'), getValue('option2'), getValue('option3'), getValue('option4')]
              : [getValue('option_a'), getValue('option_b'), getValue('option_c'), getValue('option_d')];
            const correctAnswer = activeFormat === 'subject'
            ...
            if (activeFormat === 'mixed') {
              ...
              const hasCompiler = getAnyValue('hascompi', 'hascompiler').toLowerCase() === 'true';
              const isCoding = hasCompiler || type === 'CODING';

              if (isCoding) {
                const missingCodingFields = [];
                if (!subject) missingCodingFields.push('subject/testName');
                if (!getValue('description')) missingCodingFields.push('description');
                ...
                if (missingCodingFields.length > 0) {
                  reject(`Row ${rowNumber} (CODING) is missing required value(s): ${missingCodingFields.join(', ')}.`);
                  return;
                }
                continue;
              }
            ...
            if (!text) missingFields.push(activeFormat === 'subject' ? 'text' : 'question');
            options.forEach((option, optionIndex) => {
              if (!option) {
                missingFields.push(
                  activeFormat === 'subject' ? `option${optionIndex + 1}` : `option_${String.fromCharCode(97 + optionIndex)}`
                );
              }
            });
            if (!correctAnswer) {
              missingFields.push(activeFormat === 'subject' ? 'correctAnswer' : 'correct_answer');
            }

            if (missingFields.length > 0) {
              reject(`Row ${rowNumber} is missing required value(s): ${missingFields.join(', ')}.`);
              return;
            }

            const answerMatches = options.some((option) => option.toLowerCase() === correctAnswer.toLowerCase());
            if (!answerMatches) {
              reject(`Row ${rowNumber} has an invalid correct answer. It must match one of the provided options.`);
              return;
            }
          }

          const sections = [...new Set(rows
            .map((row) => {
            ...
            if (activeFormat === 'mixed') {
              const actualHeader = normalizedHeaders.subject;
              return String(actualHeader ? row[actualHeader] ?? '' : uploadTestName.trim()).trim();
            }
  ...
  const handleUploadCSV = async () => {
    ...
    if (isPdfUpload) {
      if (!uploadTestName.trim()) {
        addToast('Please enter a Target Subject Name before uploading the PDF.', 'error'); return;
      }
      const missingPdfTestCases = Object.entries(pdfTestCases)
        .filter(([, value]) => !String(value || '').trim())
        .map(([key]) => key);
      if (missingPdfTestCases.length > 0) {
        addToast(`PDF upload requires ${missingPdfTestCases.join(', ')}.`, 'error'); return;
      }
    }
    ...
    try {
      ...
      setUploadStatus(
        isPdfUpload
          ? { ok: true, count: 1, sections: [uploadTestName || 'General'], message: 'PDF coding question uploaded successfully.' }
  ...
  const updateSection = (i, field, value) => {
    setChosenSections(prev => {
      ...
      if (field === 'subject' && copy[i].sectionMode === 'COMPILER' && copy[i].supportedLanguages.length === 0) {
        copy[i].supportedLanguages = ['Python'];
      }

      if (field === 'sectionMode' && value === 'COMPILER' && copy[i].supportedLanguages.length === 0) {
        copy[i].supportedLanguages = ['Python'];
      }
  ...
  const handleSaveAssessment = async () => {
    ...
    for (let i = 0; i < chosenSections.length; i++) {
        ...
        if (sec.sectionMode === 'COMPILER' && (!Array.isArray(sec.supportedLanguages) || sec.supportedLanguages.length === 0)) {
            addToast(`Please select at least one compiler language for Section ${i+1}.`, 'error'); return;
        }
    }
    ...
    try {
      ...
      await api.post(`${apiBase}/assessments/create`, payload, { withCredentials: true });
    ...
    <div className="tm-page">
      ...
      <div className="tm-card tm-card-upload">
        ...
        <div className="tm-format-box">
          ...
          <p className="tm-format-note"><i className="fas fa-lightbulb" /> CSV uploads can contain bulk questions. PDF uploads are treated as a coding problem statement and use the manual test cases entered below.</p>
        </div>
        ...
        {file && (
          ...
          <div className="tm-field-group">
            <label className="tm-label">PDF Test Cases <span className="tm-required">*</span></label>
            <p className="tm-field-hint"><i className="fas fa-info-circle" /> The PDF can be in any readable format. Enter two test cases here for compiler evaluation.</p>
            <div className="tm-pdf-testcase-grid">
              <textarea
                ...
                onChange={e => setPdfTestCases(prev => ({ ...prev, input1: e.target.value }))}
              />
              <textarea
                className="tm-input"
                rows="3"
                ...
                onChange={e => setPdfTestCases(prev => ({ ...prev, output1: e.target.value }))}
              />
              <textarea
                className="tm-input"
                rows="3"
                ...
                onChange={e => setPdfTestCases(prev => ({ ...prev, input2: e.target.value }))}
              />
              <textarea
                className="tm-input"
                rows="3"
                ...
                onChange={e => setPdfTestCases(prev => ({ ...prev, output2: e.target.value }))}
              />
            </div>
          </div>
        )}
        ...
        >
          <input id="tm-file-input" type="file" accept=".csv,.pdf" hidden
          ...
          {file ? (
            <div className="tm-drop-filled">
              <i
                className={`fas ${
                  (String(file.name || '').toLowerCase().endsWith('.pdf') || String(file.type || '').toLowerCase().includes('pdf'))
                    ? 'fa-file-pdf'
                    : 'fa-file-csv'
                } tm-file-icon`}
              />
            ...
            <div className="tm-drop-empty">
              ...
              <p className="tm-drop-main">Drag and drop your CSV / PDF file here</p>
        ...
        >
          {(() => {
            ...
            if (uploading) {
              return <><i className="fas fa-spinner fa-spin" /> Uploading...</>;
            }
            return <><i className="fas fa-upload" /> Upload {isPdf ? 'PDF' : 'CSV'}</>;
          })()}
        </button>
      </div>
