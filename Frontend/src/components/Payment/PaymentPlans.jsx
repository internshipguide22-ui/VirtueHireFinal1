...
function PaymentPlans() {
  ...
  const isFreeTrialActive =
    currentSubscription?.planType === "FREE_TRIAL_3_MONTHS" && !currentSubscription?.isExpired;
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", padding: "48px 20px" }}>
      <div style={{ maxWidth: "1180px", margin: "0 auto" }}>
        ...
        >
          ...
          <h2 style={{ margin: "0 0 12px", fontSize: "2.1rem" }}>Choose how long the HR module stays active</h2>
          <p style={{ margin: 0, maxWidth: "780px", lineHeight: 1.7, color: "rgba(255,255,255,0.88)" }}>
            HR access is free for the first 3 months. After that, continue with a 1 month, 3 months, or 1 year subscription.
            This subscription applies only to the HR module.
          </p>
          {isFreeTrialActive ? (
            <p style={{ margin: "14px 0 0", maxWidth: "780px", lineHeight: 1.7, color: "#bfdbfe", fontWeight: 600 }}>
              Your free 3-month trial is currently active. Paid subscriptions become available after this trial ends.
            </p>
          ) : null}
        </div>
        ...
        >
          {HR_SUBSCRIPTION_PLANS.map((plan) => {
            const isLockedByTrial = isFreeTrialActive && plan.type !== "FREE_TRIAL_3_MONTHS";
            const isCurrentPlan = currentSubscription?.planType === plan.type && !currentSubscription?.isExpired;
            const isFreeTrialCurrentPlan = isCurrentPlan && plan.type === "FREE_TRIAL_3_MONTHS";
            ...
            <article
              ...
              style={{
                ...
                borderRadius: "22px",
                padding: "24px",
                border: plan.type === "FREE_TRIAL_3_MONTHS" ? "2px solid #60a5fa" : "1px solid #e2e8f0",
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", gap: "10px", alignItems: "start" }}>
                <div>
                  <h3 style={{ margin: 0, fontSize: "1.15rem", color: "#0f172a" }}>{plan.label}</h3>
                  <p style={{ margin: "8px 0 0", color: "#64748b", fontSize: "0.92rem", lineHeight: 1.6 }}>
                    {plan.description}
                  </p>
                </div>
                {isCurrentPlan ? (
              ...
              <div style={{ fontSize: "1.9rem", fontWeight: 800, color: "#111827" }}>{plan.priceLabel}</div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "8px", color: "#334155", fontWeight: 600 }}>
                ...
                <div style={{ display: "grid", gap: "10px", color: "#475569", fontSize: "0.88rem" }}>
        ...
        <div style={{ marginTop: "24px", display: "flex", gap: "12px", flexWrap: "wrap" }}>
          <button
            type="button"
            onClick={() => navigate(currentHr ? "/hr/dashboard" : "/hrs/register")}
            style={{
